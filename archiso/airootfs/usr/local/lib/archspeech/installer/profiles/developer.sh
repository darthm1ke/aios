#!/usr/bin/env bash
# ArchAI Install Profile: Developer Workstation
# Packages: KDE, VS Code, Docker, languages, databases, tools
...
DEV_PACKAGES="code docker docker-compose nodejs npm python python-pip rustup go jdk-openjdk \
...
echo "LANG=$LOCALE" > /etc/locale.conf && echo "archai-dev" > /etc/hostname
useradd -m -G wheel,audio,video,docker,storage -s /bin/zsh "$USERNAME"
echo "$USERNAME ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers.d/$USERNAME
CHROOT

log_progress 50 "Installing KDE desktop"
run_logged "Desktop" arch-chroot /mnt pacman -S --noconfirm $DESKTOP_PACKAGES

log_progress 65 "Installing dev stack"
...
arch-chroot /mnt systemctl enable NetworkManager sddm docker postgresql \
...
log_progress 100 "Dev workstation ready. Go ship something."
umount -R /mnt
